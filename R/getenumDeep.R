#' Summarizes veris enumerations from hierarchical verisr objects
#' 
...
#'     
#' This updates getenumCI to add the ability to work on hierarchical verisr
#'     objects, including the ability to enumerate the 'sequence' column.
#'     The enumeration of any other list column is ignored as only the
#'     sequence column is ordered.  The primary difference between getenumCI
#'     and getenumDeep is getenumDeep runs some sanity checks if 'sequence'
#'     is enumerated and then flattens the hierarchical dataframe, keeping 
#'     the enumerated columns.
#'     
...
#'     
#' GetenumDeep() can also provide binomial confidence intervals for the 
#'     enumerations tested within the features.  See the parameters for details.
#'     
#' While getenumDeep() may work on other types of dataframes, it was designed for
...
#' chunk <- getenumDeep(vcdb, "action.hacking.variety")
#' chunk
#' chunk <- getenumDeep(vcdb, "action.hacking.variety", by="timeline.incident.year")
#' chunk
#' chunk <- getenumDeep(vcdb, 
#'                    "action.hacking.variety", 
#'                    by="timeline.incident.year") 
#' reshape2::acast(chunk, by~enum, fill=0)
#' getenumDeep(vcdb, "action")
#' getenumDeep(vcdb, "asset.variety")
#' getenumDeep(vcdb, "asset.assets.variety")
#' getenumDeep(vcdb, "asset.assets.variety", ci.method="wilson")
#' getenumDeep(vcdb, "asset.cloud")
#' getenumDeep(vcdb, "action.social.variety.Phishing")
#' getenumDeep(vcdb, "actor.*.motive", ci.method="wilson", na=FALSE)
getenumDeep <- function(veris, 
                      enum, 
                      by=NULL,
                      na.rm = NULL, 
                      unk=FALSE, 
  ...
  if (enum == "sequence") {
    ...
    enum <- "sequence.length"
  }
  
  
  # TODO: Get the flattened, subsetted, dataframe
  df <- flatten(df, columns=c(enum, by))
